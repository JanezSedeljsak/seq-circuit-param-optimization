...
class HyperSearch:
    ...
    def optimize_parameters(self, population_size: int, generations: int, seed: int = None):
        np.random.seed(self.best_seed if seed is None else seed)
        best_params = self.algorithm(self._evaluator, is_export=self.is_export, **self.best_params).optimize_parameters(population_size, generations)
